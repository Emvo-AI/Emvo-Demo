...
import { GeminiWebSocket } from "@/config/gemini";
import { Base64 } from "js-base64";
import { getPrompt } from "@/config/getPrompt";
...
export default function AudioChat({
  ...
  if (customPrompt) {
    ...
    currentPrompt = getPrompt(selectedAgent, selectedVoice);
  }
  ...
  useEffect(() => {
    ...
    const setupAudioProcessing = async () => {
      try {
        ...
        await ctx.audioWorklet.addModule("/audioWorker.js");
  ...
  return (
    <div className="space-y-4">
      ...
      {/* {isStreaming && connectionStatus !== "connected" && (
        <div className="inset-0 bg-black/50 flex items-center justify-center rounded-lg backdrop-blur-sm">
      ...
      <button
        onClick={toggleCamera}
        className={`px-4 py-2 mt-3 md:mt-0 rounded-full text-white hidden md:block
    ${
